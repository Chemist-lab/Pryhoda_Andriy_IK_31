  ...
  include:
    ...
    - stage: "Build Lab 4."
      ...
      install:
        - cd ./lab4/
      script:
        - docker build -t chemist1337/lab4:django-travis .
        - docker images
        - if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin; docker push chemist1337/lab4:django-travis; else echo "PR skip deploy"; fi
    - stage: "Build Lab 5."
...
```text
    - stage: "Build Lab 4."
      ...
      install:
        - cd ./lab4/
      script:
        - docker build -f Dockerfile -t chemist1337/lab4:django-travis .
        - docker build -f Dockerfile.site -t chemist1337/lab4:monitoring-travis .
        - docker images
        - if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then echo "$DOCKER_PASSWORD" | docker login -u "$DOCKER_USERNAME" --password-stdin; docker push chemist1337/lab4:django-travis; docker push chemist1337/lab4:monitoring-travis; else echo "PR skip deploy"; fi
```
...
```text
    - stage: "Build Lab 5."
      ...
      script:
        ...
        - make docker-push
```
* ##### Посилання на мій [***Travis***](https://app.travis-ci.com/github/Chemist-lab/) білд.
